...
int student::partition (int l, int r)
{
    ...
    stud pivot = s[r];//set pivot value
    int i = (l - 1);//intially i=l-1

    for (int j = l; j <= r - 1; j++)//this will run upto r-1
    {

        if (s[j].sgpa > pivot.sgpa)//condition checking in for loop if tis is true then inside code will run
        {
    ...
    return (i + 1);//value return this valus will be called to caller function
}
...
void student::quick_sgpa(int l,int r)
{
    if (l< r)//condition check
    {
        ...
        quick_sgpa(l, pi - 1);//calling sgpa function
