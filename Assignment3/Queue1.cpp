...
{
	if(front==-1 || front==rear+1)//Queue empty when no insertion takes place or all elements are deleted
...
{
	if ((front == rear+1) || ((front == 0) && (rear == tmp-1)))
...
patient Queue1::Q_delete(){//deleting element in queue
	...
	if(!(Q_isempty()))//checking queue is empty
	{
		...
		front =(front+1)%tmp;//incrementing front deleting the previous value
		if(front==(rear+1)%tmp)//if front is equal to rear set front =0
		  front=-1,rear=-1;
	}
